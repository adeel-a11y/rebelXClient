...
export async function getClientOrdersLists(
  externalId,
  {
    page = 1,
    limit = 10,
...
export async function getOrderById(id) {
  ...
  if (res.data?.success !== true)
    throw new Error(res.data?.message || "Get Order By Id failed");
  return res.data.data; // created sale order doc
}

export async function getOrdersSummary(externalId) {
  const res = await axios.get(`${BASE_URL}/sales/orders-count-by-status`, {
    params: { externalId: externalId || '' },
  });
  console.log("orders summary inside api =>", res);
  return res.data;
}

export async function createOrder(payload, { headers = {}, signal } = {}) {
  const res = await axios.post(`${BASE_URL}/sales`, payload, {
    headers,
    signal,
  });
  if (res.data?.success !== true)
...
export async function updateOrder(id, payload, { headers = {}, signal } = {}) {
  ...
  const res = await axios.put(`${BASE_URL}/sales/update/${id}`, payload, {
    headers,
    signal,
  });
  if (res.data?.success !== true)
...
export async function deleteOrder(id, { headers = {}, signal } = {}) {
  if (!id) throw new Error("Order id is required");
  const res = await axios.delete(`${BASE_URL}/sales/delete/${id}`, {
    headers,
    signal,
  });
  if (res.data?.success !== true)
