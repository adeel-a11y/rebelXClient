...
function StatusCell({ value }) {
  ...
  const cls =
    ORDER_STATUS_COLORS[key] || "bg-slate-100 text-slate-700 border-slate-200";
  return (
    <span
    ...
    const columns = useMemo(
        () => [
            ...
            { field: "State", headerName: "State", width: 140, valueFormatter: dash },
            {
                ...
                renderCell: (p) => <div>
                    {p.value === "FALSE" ?
                        <span className="inline-flex items-center px-2 py-[2px] rounded-full text-xs border bg-rose-100 text-rose-800 border-rose-200">{p.value}</span>
                        :
                        <span className="inline-flex items-center px-2 py-[2px] rounded-full text-xs border bg-sky-100 text-sky-800 border-sky-200">{p.value}</span>}
                </div>
            },
            {
                field: "TimeStamp",
